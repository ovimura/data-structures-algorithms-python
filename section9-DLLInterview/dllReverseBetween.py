...
class DoublyLinkedList:
    ...
    def reverse_between(self, start_index, end_index):
        ...
        # First node in the section to reverse
        current = prev.next
    
        # Reverse nodes by moving each next node
        # to the front of the section
        for _ in range(end_index - start_index):
            move = current.next
    
            # Remove move from its current position
            current.next = move.next
    
            if move.next is not None:
                move.next.prev = current
    
            # Insert move immediately after prev
            move.next = prev.next
            prev.next.prev = move
        ...
        # Update head and remove dummy reference
